...
export function ConfigStudioHub() {
  const links = CONFIG_STUDIO_NAV.filter((l) => l.href !== "/dashboard/admin/configuration");
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {links.map((link) => (
        ...
        >
          ...
          <p className="mt-1 text-sm text-slate-500">Configure {link.label.toLowerCase()} settings</p>
        </Link>
      ))}
    </div>
  );
