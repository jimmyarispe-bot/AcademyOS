-- 338_interest_form_v13_ga_goal_campus_gate_2026_09_12.sql
--
-- Version 13: Georgia's scholarship questions stop appearing on the high
-- school and virtual forms.
--
-- WHAT WENT WRONG. Every campus section is shown or hidden by the school the
-- family picked. The GA GOAL eligibility section was not: it was shown when
-- GA GOAL was ticked, and nothing more. That reads as equivalent, because the
-- only place GOAL can be ticked is inside the Georgia section — but ticking it
-- and then changing the school selector leaves the tick behind in the browser,
-- and a section that never asks which school this is has no reason to close.
--
-- So a parent who looked at Georgia first and then switched to the high school
-- carried Georgia's income and eligibility documents with them.
--
-- THE FIX, IN TWO PLACES. Here, the section now requires the Georgia campus as
-- well as the GOAL tick, so it cannot open anywhere else whatever is left in
-- state. In the renderer, changing the school now forgets the answers that
-- belong to sections which are no longer shown — the general form of the same
-- bug, which would otherwise wait for the next answer-gated section somebody
-- adds.
--
-- Nothing was ever at risk of being saved to the wrong campus: the server drops
-- hidden answers before it validates, so those Georgia values would have been
-- discarded on submit. The damage was to the parent, who was shown a wall of
-- irrelevant required questions and could not submit.
--
-- Identical to version 12 in every other respect.
--
-- Version 12 is archived, not deleted. Safe to re-run.
...
-- Expect version 13, 11 sections, 59 questions, 0 unresolved campus tokens.
select
