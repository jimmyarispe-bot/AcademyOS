-- 341_interest_form_v15_hs_programs_2026_09_12.sql
--
-- Version 15: the high school offers the two programmes it runs, and a family
-- claiming a scholarship is asked to show it.
--
-- 1. THE PROGRAMME LIST WAS NEVER THE FORM'S TO CHOOSE. "Program(s) of
--    Interest" is a `program_selector`, and until now that control ignored
--    whatever the question said and rendered the network's five types on every
--    campus. A high school family was offered In-Person, Full-School Program
--    and Tutoring, none of which the high school runs.
--
--    The question now carries its own options where a campus needs a narrower
--    list, and the renderer uses them. The five network types remain the
--    default for any question that does not say otherwise, which is every other
--    campus.
--
--    THE SERVER WAS NARROWED TOO, in the same ship. A page showing two choices
--    while the validator accepted all five is the kind of gap where a posted
--    form records interest in a programme the campus does not run — and it
--    would have looked fine in testing, because nobody testing by hand posts
--    the form directly.
--
-- 2. A SCHOLARSHIP CLAIMED IS NOW A SCHOLARSHIP EVIDENCED. Both the high school
--    and Virtual ask whether the family holds a state, district or other
--    government scholarship. Answer yes and an upload appears, asking for the
--    award letter — the same thing Florida and Georgia already ask for, and for
--    the same reason: the figure decides what the family owes.
--
--    No new machinery. `file` questions, the upload route and the attachment to
--    the lead all arrived in version 12; these two questions just use them.
--
-- 3. "How many years of high school have you attended?" now reads "…have your
--    child attended?". Version 14 moved that question out of the student's
--    section and left it addressed to the student, in a form their parent is
--    filling in.
--
-- Version 14 is archived, not deleted. Safe to re-run.
...
-- Expect version 15, 10 sections, 58 questions, 0 unresolved campus tokens.
select
