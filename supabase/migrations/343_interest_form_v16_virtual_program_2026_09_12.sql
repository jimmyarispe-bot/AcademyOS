-- 343_interest_form_v16_virtual_program_2026_09_12.sql
--
-- Version 16: Virtual offers the one programme it runs.
--
-- Version 15 gave the high school its own two-option programme question and
-- left every other campus on the network's five. Virtual runs one of them, so
-- it now names it: "Only Virtual", and nothing else.
--
-- The network question steps aside for both campuses that name their own. It
-- still shows before a school is chosen and on Florida and Georgia, which is
-- where the five types are still the honest answer.
--
-- A single-option list is deliberate rather than an oversight. The question is
-- required, so the family ticks the one thing on offer and the answer is
-- recorded — where dropping the question would have left the record silent
-- about a campus whose whole premise is how it is delivered.
--
-- Nothing else changes. Virtual's own "What program are you interested in for
-- your child?" — full-school versus the four tutoring tracks — is a different
-- question and is untouched.
--
-- Version 15 is archived, not deleted. Safe to re-run.
...
-- Expect version 16, 10 sections, 59 questions, 0 unresolved campus tokens.
select
