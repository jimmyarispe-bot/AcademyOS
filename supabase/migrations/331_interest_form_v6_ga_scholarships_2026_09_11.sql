-- 331_interest_form_v6_ga_scholarships_2026_09_11.sql
--
-- Version 6: Georgia's scholarship questions ask what Georgia actually asks.
--
-- WHAT CHANGES
--
--   1. GA SPECIAL NEEDS now collects the STUDENT ID as well as the award
--      amount, and both appear only when that scholarship is ticked. The award
--      amount was previously visible to every Georgia family whether or not it
--      applied to them.
--
--   2. GA GOAL gets its own section carrying the eligibility rule in full, in
--      the law's own words, with a checkbox for every route to qualifying — the
--      two base paths and the five waivers of the six-week attendance
--      requirement. It appears only when GA GOAL is ticked.
--
--   3. The GA GOAL narrative now appears for GA GOAL and Academy-Based
--      applicants only, which is what the paper form says and what the question
--      text already claimed.
--
-- THE OPERATOR THIS DEPENDS ON. `ga_scholarships` is a multiselect, and until
-- today no condition could ask what a multiselect contained. `eq` compares an
-- array to a string and is never true; `in` asks whether the ANSWER appears in
-- a list supplied by the rule, which is the opposite question. A `contains`
-- operator ships with this migration. Without it these questions would have to
-- be shown to every Georgia family regardless of what they are applying for.
--
-- THE AWARD LETTER IS NOT HERE, AND THAT IS THE DECISION ALREADY MADE. The
-- Special Needs award letter is a document, the form engine has no file field,
-- and uploads now belong to the application stage — where a parent has an
-- account, DocumentCenter already works, and no anonymous browser is handed
-- storage credentials. The student ID and amount are collected here because
-- they are the two values staff need to reconcile an award without opening the
-- letter at all.
--
-- The low-performing schools list is linked as plain text because helpText
-- renders as plain text. The Georgia paper form does the same thing with its
-- award calculator link, so families already expect to copy an address.
--
-- Version 5 is archived, not deleted. Safe to re-run.
  ...
  definition_text := $json$
{
  ...
  "questions": [
    ...
    {
      "key": "ga_special_needs_student_id",
      "type": "text",
      "label": "GA Special Needs Scholarship — student ID",
      "required": false,
      "order": 47,
      "systemBinding": null,
      "helpText": "The student ID on your award letter.",
      "visibleWhen": {
        "all": [
          {
            "path": "ga_scholarships",
...
-- Expect version 6, 10 sections, 56 questions, 0 unresolved campus tokens.
select
