-- 339_interest_form_v14_hs_student_handoff_2026_09_12.sql
--
-- Version 14: the high school stops asking the student to fill in the parent's
-- form, and starts asking the same scholarship question as Virtual.
--
-- THREE CHANGES, ALL TO THE HIGH SCHOOL.
--
-- 1. The "Student Section" is gone — five narrative questions the student was
--    meant to answer in their own words, sitting in the middle of a form their
--    parent is filling in. In practice that means a parent typing answers on
--    behalf of a teenager who is not in the room, which is worth less than
--    asking nobody.
--
-- 2. The student's email address carries the handoff instead, and now says so:
--    "Your student will be sent a few questions for him/her to complete once
--    you submit this form."
--
--    THAT EMAIL DOES NOT EXIST YET. The address is collected and stored; no
--    automation sends the questions. Until it is built, the sentence is a
--    promise the system does not keep — somebody has to send those questions by
--    hand, or the next piece of work is building it.
--
--    The five questions are preserved in version 13, which is archived rather
--    than deleted, so whoever builds the follow-up has the exact wording.
--
-- 3. The high school now asks the scholarship question Virtual already asked:
--    "I/we have a scholarship from our state, district or other government
--    source that we will be using to help offset the tuition cost." Same
--    wording, same two answers, so the two campuses stay comparable.
--
-- Section orders close up behind the removed section. Nothing else changes:
-- Florida, Georgia and Virtual are untouched.
--
-- Version 13 is archived, not deleted. Safe to re-run.
...
-- Expect version 14, 10 sections, 55 questions, 0 unresolved campus tokens.
select
