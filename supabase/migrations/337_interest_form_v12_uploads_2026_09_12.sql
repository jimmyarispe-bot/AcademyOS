-- 337_interest_form_v12_uploads_2026_09_12.sql
--
-- Version 12: families can actually attach the documents the form asks for.
--
-- Until now the GA GOAL section NAMED the documents and offered nowhere to put
-- them. That was honest about the constraint and useless to a parent, who read
-- "you will be asked to provide proof of income" and reasonably looked for a
-- button.
--
-- WHAT IS NOW COLLECTED
--   FL   the Step Up award letter or screenshot, when a scholarship is named
--   GA   the GA Special Needs award letter, when that scholarship is ticked
--   GA   proof of income and proof of eligibility, when GA GOAL is ticked
--
-- AND A SECOND SIGNATURE. GA GOAL applicants now sign that the eligibility
-- criteria they ticked are true and the documents they uploaded are genuine.
-- The taxpayer pledge in v10 is a different thing — that one is about
-- supporting the fund and every Georgia family signs it. This one attests to
-- claims that decide whether a scholarship is awarded, and only the families
-- making those claims see it.
--
-- HOW UPLOADS WORK, BECAUSE IT IS NOT OBVIOUS. The file goes to
-- /api/apply/upload the moment it is chosen, before the form is submitted, and
-- the answer stored in the form is the storage path that route returns. That
-- ordering is deliberate: a parent learns their file is too large while looking
-- at the field, rather than after filling in everything else.
--
-- The path is validated on submit against the exact shape the upload route
-- issues. A client posting the form directly could otherwise name any object in
-- the bucket and have it attached to their own inquiry, which is somebody
-- else's document handed to a stranger.
--
-- REQUIRES MIGRATION 326, WHICH HAS NOT BEEN RUN. On submit each uploaded path
-- becomes a row in `application_documents` against the LEAD, because no
-- application exists yet. 326 adds `lead_id` and — the part that matters —
-- rewrites the staff read policy to resolve a school through the lead when
-- there is no application. Without it those rows are invisible to every member
-- of staff: an upload that succeeded, reported success, and was seen by nobody.
--
-- RUN 326 FIRST. Then ship the code. Then run this.
--
-- Version 11 is archived, not deleted. Safe to re-run.
...
-- Expect version 12, 11 sections, 59 questions, 0 unresolved campus tokens.
select
