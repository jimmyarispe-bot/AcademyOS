-- 334_interest_form_v9_currency_2026_09_11.sql
--
-- Version 9: money looks like money, and Georgia's GTID says whose it is.
--
--   1. `fl_scholarship_amount` and `ga_special_needs_amount` become the new
--      `currency` type. They render as a number field with a dollar sign inside
--      it, on the left, where a parent copying a figure off an award letter can
--      see what the box wants while they are typing into it. A label reading
--      "amount ($)" is read once and forgotten.
--
--      The stored value is unchanged — a plain number. The symbol is decoration
--      and nobody has to type or delete it. The submit validator now also
--      strips a typed "$" and any commas, because a parent copying
--      "$10,463.00" off the letter in front of them is reading correctly, and
--      being told that is not a valid number is how a form loses people.
--
--   2. "GTID" becomes "GA Special Needs Scholarship GTID". On its own the
--      acronym sits under a heading and leaves a parent to infer which of their
--      scholarships it belongs to. The question names the programme now.
--
-- The key stays `ga_gtid`. It was renamed in v7 an hour ago because nothing had
-- been answered under it; the same is still true, but a key should stop moving
-- once it is right, and `ga_gtid` is right.
--
-- REQUIRES THE CODE THAT RENDERS `currency`. Ship first: an unknown field type
-- falls through to the generic text input, which works but shows no symbol.
--
-- Version 8 is archived, not deleted. Safe to re-run.
...
-- Expect version 9, 11 sections, 53 questions, 0 unresolved campus tokens.
select
