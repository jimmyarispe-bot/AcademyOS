-- 346_interest_form_v17_virtual_program_position_2026_09_12.sql
--
-- Version 17: Virtual's programme question moves up next to the other
-- programme questions.
--
-- "What program are you interested in for your child?" — full-school or one of
-- the four tutoring tracks — was at the bottom of the form, in the Virtual
-- section, several screens below "Program(s) of Interest". Two questions about
-- what the family wants, asked at opposite ends of the page. It now sits
-- immediately above "How did you hear about our school?", with the rest of the
-- programme questions.
--
-- IT NEEDS ITS OWN GATE NOW. Inside `virtual_detail` it was shown or hidden by
-- the section, which asks whether the family chose Virtual. Program & School is
-- shown to everybody, so the question carries that condition itself — otherwise
-- every campus would be asked which Virtual tutoring track they wanted.
--
-- That is the general hazard when a question moves between sections, and it is
-- silent: the question simply appears where it should not, on a form nobody
-- thinks to re-check because the change was described as moving something up.
--
-- Nothing else changes. Same question, same five options, same answer key, so
-- answers already recorded still mean what they meant.
--
-- Version 16 is archived, not deleted. Safe to re-run.
...
-- Expect version 17, 10 sections, 59 questions, 0 unresolved campus tokens.
select
